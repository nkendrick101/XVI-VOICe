...
def set_reminder(engine, command):
    ...
    reminder = listen()
    speak(engine, "When do you want to be reminded? Please say the time in 
hours and minutes.")
    ...
    try:
        ...
        speak(engine, f"Alright, I will remind you about '{reminder}' at 
{hour:02d}:{minute:02d}.")
        while True:
        ...
        speak(engine, "Sorry, I couldn't understand the time you provided. 
Please try again.")


def create_todo_list(engine, command):
    todo_list = []
    speak(engine, "Let's create a to-do list. Please say the tasks one by 
one. Say 'done' when you're finished.")
    while True:
        task = listen()
        if task == "done":
            break
...
def show_help(engine):
    ...
    1. Set reminders: Say 'set reminder' followed by the reminder and 
time.
    2. Create to-do lists: Say 'create to-do list' and then list your 
tasks one by one.
...
def main():
    ...
    speak(engine, "Hello, I am your voice assistant. How can I help you 
today?")
